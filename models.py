...
class TextEncoder(nn.Module):
  ...
  def forward(self, enc_text, enc_text_lengths):
    x = self.emb(enc_text) * math.sqrt(self.hidden_channels) # [b, t, h]
    transposed_x = torch.transpose(x, 1, -1) # [b, h, t]
    x_mask = torch.unsqueeze(commons.sequence_mask(enc_text_lengths, transposed_x.size(2)), 1).to(x.dtype)

    x = self.encoder(transposed_x * x_mask, x_mask)
